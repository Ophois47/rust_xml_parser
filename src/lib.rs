...
#[allow(dead_code)]
fn map<'a, P, F, A, B>(parser: P, map_fn: F) -> impl
	Parser<'a, B>
where
	P: Parser<'a, A>,
	F: Fn(A) -> B,
...
where
	P: Parser<'a, A>,
{
	right(one_or_more_whitespace(), left(parser, one_or_more_whitespace()))
}
...
fn quoted_string_parser() {
	assert_eq!(
		Ok(("", "Hello Dave!".to_string())),
		quoted_string().parse("\"Hello Dave!\"")
	);
}
